...
(function svl() {
  const consoleDebug =
    localStorage.getItem('svlDebugOn') === 'true'
      ? (...args) => {
  ...
  function savePreferences(pref) {
    ...
    localStorage.setItem('svl', JSON.stringify(pref));
  }
  ...
  function drawLabels(model, simplified, delayed = false) {
    ...
    const hasStreetName = model.hasNonEmptyStreet();
    if (
      ...
      } else if (attributes.roadType < 10 && !model.isInRoundabout()) {
        streetPart = '⚑';
      }
      ...
      if (preferences.showANs) {
        ...
        for (let streetID of attributes.streetIDs) {
          ...
          const altStreet = model.model.streets.objects[streetID];
      ...
      const sampleLabel = new OpenLayers.Feature.Vector(simplified[0], {
        myId: attributes.id,
        color: streetStyles[attributes.roadType]
          ? streetStyles[attributes.roadType].strokeColor
          : '#f00',
        outlinecolor: streetStyles[attributes.roadType]
          ? streetStyles[attributes.roadType].outlineColor
          : '#fff',
        ...
        if (distance >= thresholdDistance) {
          ...
          if (distance > streetNameThresholdDistance) {
            ...
            if (distance >= doubleLabelDistance) {
              ...
              p0 = p1;
              p1 = simplified[p + 1];
              centroid = new OpenLayers.Geometry.LineString([
                p0,
                p1,
              ]).getCentroid(true);
    ...
    if (streetStyles[roadType] !== undefined) {
      ...
      if (
        ...
        if ((attributes.fwdDirection || attributes.revDirection) === false) {
          ...
          const step = isInRoundabout ? 3 : 1; // It is a roundabout
          for (let p = step - 1; p < simplifiedPoints.length - 1; p += step) {
            ...
            const segmentLength = simplifiedPoints[p].distanceTo(
              simplifiedPoints[p + 1]
            );
            // TODO: don't refer to zoom here!
            const minDistance = 15.0 * (11 - OLMap.zoom);
            if (segmentLength < minDistance * 2) {
              ...
              const dx = simplifiedPoints[p + 1].x - simplifiedPoints[p].x;
              const dy = simplifiedPoints[p + 1].y - simplifiedPoints[p].y;

              const numPoints =
                Math.floor(Math.sqrt(dx * dx + dy * dy) / minDistance) - 1;

              const stepx = dx / numPoints;
              const stepy = dy / numPoints;
              let px = simplifiedPoints[p].x + stepx;
              let py = simplifiedPoints[p].y + stepy;
              for (let ix = 0; ix < numPoints; ix += 1) {
                myFeatures.push(
                  new OpenLayers.Feature.Vector(
      ...
      if (preferences.renderGeomNodes && !isInRoundabout) {
  ...
  function drawNode(model) {
    const point = new OpenLayers.Geometry.Point(
      model.attributes.geometry.x,
      model.attributes.geometry.y
    );
  ...
  function nodeStateDeleted(e) {
    ...
    for (let i = 0; i < e.length; i += 1) {
      removeNodeById(e[i].getID());
    }
  }

  function segmentsStateDeleted(e) {
    for (let i = 0; i < e.length; i += 1) {
      removeSegmentById(e[i].getID());
    }
  }
  ...
  function initSVL(svlAttempts = 0) {
    ...
    console.log(
      `Street Vector Layer v. ${GM_info.script.version} initialized correctly.`
    );
  }
  ...
  function updateStylesFromPreferences(pref) {
    let i;
    let len;
    for (i = 0, len = pref.streets.length; i < len; i += 1) {
      if (pref.streets[i]) {
        streetStyles[i] = {
